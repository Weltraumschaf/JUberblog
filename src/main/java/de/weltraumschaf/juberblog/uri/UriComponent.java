...
class UriComponent {
    ...
    static void validate(String s, Type t, boolean template) {
        ...
        if (i > -1) {
            throw new IllegalArgumentException(
                    String.format(
                            "The string \"%s\" for the URI component %s contains an invalid character, ''%s'', at index %s.",
                            s, t, s.charAt(i), i));
        }
    }
    ...
    static String decode(String s, Type t) {
        ...
        if (s.indexOf('%') < 0) {
            ...
            if (n < 2) {
                throw new IllegalArgumentException(
                        String.format("Malformed percent-encoded octet at index %d!", 1));
            }

            // Malformed percent-escaped octet at the end
            if (s.charAt(n - 2) == '%') {
                throw new IllegalArgumentException(
                        String.format("Malformed percent-encoded octet at index %d!", n - 2));
            }
        }
