...
final class UriParser {
    private static final String ERROR_STATE = "URI parser not executed!";
    ...
    private String parseComponent(String delimiters, boolean mayEnd, boolean isIp) {
        ...
        throw new IllegalArgumentException(
                String.format("URI parser component delimiter '%s' at %d!", delimiters, ci.pos()));
    }
    ...
    public void parse() {
        ...
        if (ci.current() == ':') {
            ...
            if (comp == null) {
                throw new IllegalArgumentException(
                        String.format("URI parser scheme expected at %d for '%s'!", ci.pos(), input));
            }
            scheme = comp;
            if (!ci.hasNext()) {
